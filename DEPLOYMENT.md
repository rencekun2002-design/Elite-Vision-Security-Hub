...
3. Import `evs-drill-simulator.sql` once to create the schema and seed data.
4. If the database already existed before this update, run `migrations/001_add_history_detail.sql`
   once in phpMyAdmin to enable full drill breakdowns in History.
5. Run `migrations/002_add_police_hotline.sql` once to add admin-maintained police contact fields.
6. Run `migrations/003_import_client_hotlines.sql` once to import the client addresses,
   response equipment, police departments, and hotline data from the reference PDF.
7. Set `EVS_DB_HOST`, `EVS_DB_NAME`, `EVS_DB_USER`, and `EVS_DB_PASS` in the Apache/PHP
   environment. The application keeps the local XAMPP defaults when these variables
   are not set.
8. Change the `ADMIN_CODE` value in `evs-drill-simulator.html` before production deployment.
